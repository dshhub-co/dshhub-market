...
import { nodeBinDir, cmdCommandLine } from './dsh-cli.js';
import { logEvent } from './log.js';
import { appsRoot, readInstalledApps, runtimeStatePath } from './app-install.js';
 ...
 */
export function appStatus(name) {
...
export function listAppStatus() {
    const out = {};
    for (const name of Object.keys(readInstalledApps()))
        out[name] = appStatus(name);
    return out;
}
// ---- 命令解析与运行时探测 ----
...
function runCommand(record, command, timeoutMs) {
 ...
 */
export async function deployApp(name) {
    const app = readInstalledApps()[name];
    if (app === undefined)
        throw new Error(`应用未安装：${name}（请先在解锁卡点「安装」）`);
