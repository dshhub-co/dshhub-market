...
export function dshhubIdFromZip(zipUrl) {
    if (!zipUrl)
        return null;
    const match = /\/([0-9a-f]{8}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{12})(?:\/|$)/i.exec(zipUrl);
    return match?.[1] ?? null;
}
/**
 ...
 */
export function blacklistHas(entries, entry) {
    if (entries.length === 0)
        return false;
