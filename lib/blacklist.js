...
import { blacklistHas } from './blacklist-util.js';
export { blacklistHas, dshhubIdFromZip } from './blacklist-util.js';
/** 可被环境变量覆盖（测试/e2e 指向本地 fixture）；默认平台生产端点。 */
export const BLACKLIST_URL = process.env.DSHM_BLACKLIST_URL ?? 'https://www.dshhub.co/api/blacklist';
const TTL_MS = 60_000;
let cached = null;
/** 拉取黑名单（带内存缓存）；失败时返回旧缓存或空列表，绝不抛。 */
export async function fetchBlacklist(force = false) {
    const now = Date.now();
    if (!force && cached !== null && now - cached.at < TTL_MS)
...
/** 测试用：清空缓存，强制下次重新拉取。 */
export function forgetBlacklist() {
    cached = null;
}
...
export async function blacklistHasName(name) {
