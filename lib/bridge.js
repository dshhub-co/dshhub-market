...
import { profileDir, dshHome } from './profile.js';
import { openStandardDir } from './cloud-bridge.js';
    ...
    return createServer((req, res) => {
        ...
        if (req.method === 'POST' && url === '/dsh-market/open-dir') {
            ...
            readBody(req)
                ...
                if (which === null)
                    return { ok: false, error: 'invalid which' };
...
export async function startBridge(profile) {
    ...
    for (const port of PORTS) {
        ...
        if (health) {
            ...
            console.warn(`[dshhub-market] port ${port} is held by another DSH instance (home: ${String(health.home ?? '?')}, profile: ${String(health.profile ?? '?')}) — binding the next free port instead`);
        }
        try {
