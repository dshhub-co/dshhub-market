 ...
 */
export async function bindToAccount(code) {
    const c = typeof code === 'string' ? code.trim() : '';
    if (!/^\d{6}$/.test(c))
        return { ok: false, error: '配对码为 6 位数字' };
    ...
    if (!candidates.includes('web'))
        candidates.unshift('web');
...
export function openInFileManager(dir) {
    try {
        if (!dir || !existsSync(dir) || !statSync(dir).isDirectory()) {
            return { ok: false, error: '目录不存在或不可访问' };
        }
        const [cmd, ...args] = process.platform === 'darwin' ? ['open', dir]
 ...
 * 目录不存在时返回提示（不报错）。
 */
export function openStandardDir(which) {
 ...
 */
const POLL_INTERVAL_MS = Number(process.env.DSHHUB_POLL_INTERVAL_MS) || 30_000; // 界面打开时 30s 基础间隔（0.8.57）：任务响应延迟可接受，请求量降 3 倍
...
export async function pollOnce(state) {
    ...
    if (min !== '' && isOlderVersion(readOwnVersion(), min)) {
        console.warn(`[dshhub-market] 本插件 v${readOwnVersion()} 低于要求的最低版本 v${min}，` +
