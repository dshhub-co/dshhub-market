...
function unlockedFile(profileDir) {
...
export function readUnlockedState(profileDir) {
    try {
        const raw = JSON.parse(readFileSync(unlockedFile(profileDir), 'utf8'));
        return {
...
export function writeUnlockedState(profileDir, state) {
    mkdirSync(join(profileDir, HOT_DIR), { recursive: true, mode: 0o700 });
 ...
 */
export function removeUnlockedRecord(profileDir, id) {
    const state = readUnlockedState(profileDir);
    const next = state.bundles.filter((b) => b.id !== id);
    if (next.length === state.bundles.length)
        return false;
 ...
 */
export function getOrCreateProfileKey(profileDir) {
    const state = readUnlockedState(profileDir);
    if (state.profileKey !== '')
...
export async function hotMount(ctx, profileDir, packageName) {
    try {
        ...
        ctx.logger?.info?.(`[dshhub-market] hot-mounted ${packageName}`);
        logEvent('info', 'hot-mount', `${packageName}: live${shimNames.has(packageName) ? ' (client-only shim)' : ''}`);
        return { ok: true, reason: null };
    }
    catch (error) {
        const message = error instanceof Error ? error.message : String(error);
        ctx.logger?.warn(`[dshhub-market] hot mount of ${packageName} failed, restart required: ${message}`);
