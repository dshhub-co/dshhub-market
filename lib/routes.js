    ...
    const disposers = [
        ...
        host.webServer.register({
            ...
            handler: async (request, response) => {
                ...
                // under <dsh-home>/.agent-presets/ — not npm packages — merged into
                // the same map with a `skill:`/`preset:` spec so the UI's
                // install/uninstall/identity matching keeps working. (Presets were
                // missing here, which made the unlocked-card button flip back to 安装
                // right after a successful preset install: refreshInstalled() never
                // saw it.)
