...
import { blacklistHasName, fetchBlacklist, isBlacklisted } from './blacklist.js';
import { cleanHotDir, getOrCreateProfileKey, hotMount, hotUnmount, listHotMounts, mountClientOnlyDeps, purgeMarketState, readMarketState, readUnlockedState, removeUnlockedRecord, writeMarketState, writeUnlockedState, } from './hot.js';
...
import { entryNeedsZip, materializeTgz, zipKind } from './zip-source.js';
import { installSkill, isInstalledSkill, skillSpecMap, uninstallSkill, readInstalledSkills } from './skill-install.js';
import { installPreset, isInstalledPreset, presetSpecMap, uninstallPreset, readInstalledPresets } from './preset-install.js';
import { appSpecMap, installApp, isInstalledApp, readInstalledApps, uninstallApp } from './app-install.js';
import { deployApp, listAppStatus, stopApp } from './app-runtime.js';
...
import { checkUpdates, fetchNpmLatest, invalidateUpdates, isLocalPathSpec, isUpgrade, latestPublishedRecently, versionOnChannel } from './updates.js';
import { FORK_SELF_NAME, fetchOwnVersion, selfUpdateTarget, updateBase } from './self-update.js';
...
const SELF_NAMES = new Set(['dshhub-market', 'dshmarket', 'dsh-market']);
/**
...
export function mountMarketRoutes(host, config, commandRuntime, agentsLookup) {
    ...
    const disposers = [
            ...
            handler: async (request, response) => {
                ...
                try {
                    ...
                    const record = {
                        ...
                        // 平台下架标记转换：status === 'removed' 的条目带 removed=true（无 zip，
                        // 解锁卡片显示「已下架」占位）；sha256/removedReason 原样透传（哈希存证）
                        items: (upstreamBody.bundle.items ?? []).map((it) => ({
                            ...it,
                            removed: it.status === 'removed' ? true : undefined,
                        })),
        ...
        host.webServer.register({
            kind: 'exact',
            path: '/dsh-market/blacklist',
            handler: async (_request, response) => {
                try {
                    // 浏览器端黑名单校准走本地代理：直接 import blacklist.ts 会把
                    // net.ts → undici → node:assert 带进浏览器 bundle，宿主加载失败。
                    // 服务端 fetchBlacklist 自带 60s TTL 缓存，与平台 CDN 对齐。
        ...
        host.webServer.register({
            ...
            handler: async (request, response) => {
                ...
                try {
                    ...
                    if (pluginId === '' || category === '' || description === '') {
                        sendJson(response, 400, { error: '举报信息不完整 / report information incomplete' });
                        return;
                    }
                    // 平台限速在服务端（report:ip 5/时 + report:plugin 10/时），本地只透传；
                    // 状态码原样透传（400/404/429），错误文案优先用平台的中文提示
                    const upstream = await fetch(`${DSHHUB_API}/api/reports`, {
                        method: 'POST',
                        headers: { 'content-type': 'application/json' },
                        body: JSON.stringify({
                            plugin_id: pluginId,
                    ...
                    const upstreamBody = (await upstream.json().catch(() => ({})));
                    if (!upstream.ok) {
                        logEvent('warn', 'report', `plugin ${pluginId}: rejected by upstream (${upstream.status})`);
                        sendJson(response, upstream.status, {
                            error: typeof upstreamBody.error === 'string' && upstreamBody.error !== ''
                                ? upstreamBody.error
                                : '举报提交失败，请稍后重试 / report submission failed, please retry in a moment',
                        });
                        return;
                    }
                    logEvent('info', 'report', `plugin ${pluginId}: report submitted`);
                    sendJson(response, 200, { ok: true });
                }
                catch (error) {
                    logEvent('error', 'report', `report failed: ${error instanceof Error ? error.message : String(error)}`);
                    const message = error instanceof Error ? error.message : String(error);
                    if (error instanceof Error && (error.name === 'TimeoutError' || error.name === 'AbortError' || /timeout/i.test(message))) {
                        sendJson(response, 504, { error: '举报提交超时，请稍后重试 / report submission timed out, please retry in a moment' });
                        return;
                    }
                    sendJson(response, 502, { error: message });
                }
        ...
        host.webServer.register({
            ...
            handler: async (request, response) => {
                ...
                try {
                    ...
                    if (pluginId === '' || !Number.isInteger(score) || score < 1 || score > 5) {
                        sendJson(response, 400, { error: '评分信息不完整 / rating information incomplete' });
                        return;
                    }
                    // 平台限速在服务端（rating:ip 20/时 + rating:plugin:{id}:{ip} 3/天），
                    // 本地只透传；状态码原样透传（400/404/429）
                    const upstream = await fetch(`${DSHHUB_API}/api/ratings`, {
                        method: 'POST',
                        headers: { 'content-type': 'application/json' },
                        body: JSON.stringify({
                            plugin_id: pluginId,
                    ...
                    const upstreamBody = (await upstream.json().catch(() => ({})));
                    if (!upstream.ok) {
                        logEvent('warn', 'ratings', `plugin ${pluginId}: rejected by upstream (${upstream.status})`);
                        sendJson(response, upstream.status, {
                            error: typeof upstreamBody.error === 'string' && upstreamBody.error !== ''
                                ? upstreamBody.error
                                : '评分提交失败，请稍后重试 / rating submission failed, please retry in a moment',
                        });
                        return;
                    }
                    logEvent('info', 'ratings', `plugin ${pluginId}: rated ${score}`);
                    sendJson(response, 200, { ok: true });
                }
                catch (error) {
                    logEvent('error', 'ratings', `ratings failed: ${error instanceof Error ? error.message : String(error)}`);
                    const message = error instanceof Error ? error.message : String(error);
                    if (error instanceof Error && (error.name === 'TimeoutError' || error.name === 'AbortError' || /timeout/i.test(message))) {
                        sendJson(response, 504, { error: '评分提交超时，请稍后重试 / rating submission timed out, please retry in a moment' });
                        return;
                    }
                    sendJson(response, 502, { error: message });
                }
        ...
        host.webServer.register({
            ...
            handler: async (_request, response) => {
                try {
                    ...
                    if (changed)
                        writeUnlockedState(activeProfileDir, { ...state, bundles });
        ...
        host.webServer.register({
            ...
            handler: async (request, response) => {
                ...
                // manifest v2: skill/preset/app packages are directories — skills under
                // <dsh-home>/skills/ (DSH only loads skills from this root), presets
                // under <dsh-home>/.agent-presets/, apps under <dsh-home>/apps/ — not
                // npm packages — merged into the same map with a `skill:`/`preset:`/
                // `app:` spec so the UI's install/uninstall/identity matching keeps
                // working. (Presets were missing here, which made the unlocked-card
                // button flip back to 安装 right after a successful preset install:
                // refreshInstalled() never saw it.)
                const skillSpecs = skillSpecMap(activeProfileDir);
                const presetSpecs = presetSpecMap(activeProfileDir);
                const appSpecs = appSpecMap();
                const installed = { ...readInstalled(config.profile, activeProfileDir), ...skillSpecs, ...presetSpecs, ...appSpecs };
                const repoIdentities = {};
                const repoHints = {};
                for (const [name, spec] of Object.entries(installed)) {
                    if (spec.startsWith('skill:') || spec.startsWith('preset:') || spec.startsWith('app:'))
                        continue;
                ...
                const present = Object.keys(installed).filter(name => installed[name].startsWith('skill:')
                    || installed[name].startsWith('preset:')
                    || installed[name].startsWith('app:')
                ...
                for (const name of Object.keys(installed)) {
                    activation[name] = installed[name].startsWith('skill:')
                        ...
                        : installed[name].startsWith('preset:')
                            ...
                            : installed[name].startsWith('app:')
                                ? { state: 'inert', reasons: ['应用包：独立进程（~/.dsh/apps），不走 DSH 插件体系，请在解锁卡「部署」使用'], bundle: false, hot: false }
                ...
                for (const [name, spec] of Object.entries(installed)) {
                    if (spec.startsWith('skill:'))
                        kinds[name] = 'skill';
                    else if (spec.startsWith('preset:'))
                        kinds[name] = 'preset';
                    else if (spec.startsWith('app:'))
                        kinds[name] = 'app';
                    else {
                        const m = readInstalledManifest(config.profile, name, activeProfileDir);
                        if (m !== null && typeof m.kind === 'string' && m.kind !== '')
                            kinds[name] = m.kind;
                    }
                }
        ...
        host.webServer.register({
            ...
            handler: async (request, response) => {
                ...
                try {
                    ...
                    if (name === 'dsh-market' || name === 'dshmarket' || name === 'dshhub-market') {
            ...
            handler: async (request, response) => {
                ...
                try {
                    await withMutationLock(response, 'install', async () => {
                        ...
                        // 下架拦截：被平台下架的插件（含 github 导入条目，按包名匹配）
                        // 不允许更新；市场本体自更新（FORK_SELF_NAME）跳过。
                        if (name !== FORK_SELF_NAME && (await blacklistHasName(name))) {
                            logEvent('warn', 'update-blocked', `${name}: blacklisted (removed from marketplace)`);
                            sendJson(response, 409, {
                                error: '该插件已被平台下架，无法更新 / This plugin has been removed from the marketplace and cannot be updated.',
                            });
                            return;
                        }
                        ...
                        // fork: the fork updates itself from the dshhub.co tarball
                        // channel, never from npm (dshhub-market is not published there).
                        const selfTgz = name === FORK_SELF_NAME;
                    ...
                    const message = error instanceof Error ? error.message : String(error);
                    host.logger?.warn(`[dshhub-market] update failed: ${message}`);
            ...
            handler: async (request, response) => {
                ...
                try {
                    await withMutationLock(response, 'install', async () => {
                        ...
                        const installed = readInstalled(config.profile, activeProfileDir);
                        const selfName = ['dshhub-market', 'dshmarket', 'dsh-market'].find(candidate => installed[candidate] !== undefined);
                ...
                try {
                    await withMutationLock(response, 'install', async () => {
                        ...
                        if (isInstalledSkill(activeProfileDir, name)) {
                            ...
                            logEvent(removed ? 'info' : 'error', 'uninstall', `${name}: skill bundle ${removed ? `removed (${record?.skills.join(', ') ?? ''})` : 'not found'}`);
                        ...
                        if (isInstalledPreset(activeProfileDir, name)) {
                            ...
                            logEvent(removed ? 'info' : 'error', 'uninstall', `${name}: preset bundle ${removed ? `removed (${record?.presets.join(', ') ?? ''})` : 'not found'}`);
                            sendJson(response, removed ? 200 : 400, {
                                ok: removed,
                                hot: true,
                                preset: record,
                        ...
                        if (isInstalledApp(name)) {
                            ...
                            logEvent(removed ? 'info' : 'error', 'uninstall', `${name}: app bundle ${removed ? `removed (v${record?.version ?? '?'})` : 'not found'}`);
                            sendJson(response, removed ? 200 : 400, {
                                ok: removed,
                                hot: true,
                                app: record,
                    ...
                    const message = error instanceof Error ? error.message : String(error);
                    host.logger?.warn(`[dshhub-market] uninstall failed: ${message}`);
        ...
        host.webServer.register({
            ...
            handler: async (request, response) => {
                ...
                try {
                    ...
                    const message = error instanceof Error ? error.message : String(error);
                    host.logger?.warn(`[dshhub-market] rollback failed: ${message}`);
                ...
                try {
                    await withMutationLock(response, 'install', async () => {
                        ...
                        if (entry === undefined) {
                            ...
                            const match = unlockedItems.find(it => typeof it.zip === 'string' && it.zip.toLowerCase() === url.toLowerCase());
                            if (match !== undefined) {
                                // 已下架条目：平台不再提供 zip（removed 条目无 zip 字段，正常
                                // 不会匹配到这里）；防御性拒绝——旧解锁记录的遗留按钮仍可能触发。
                                if (match.removed === true) {
                                    logEvent('warn', 'install-rejected', `unlocked item removed: ${match.name ?? url.slice(0, 80)}`);
                                    sendJson(response, 400, {
                                        error: '该条目已被平台下架，无法安装 / This item has been removed from the marketplace.',
                                    });
                                    return;
                                }
                                // 旧解锁卡片（平台补 kind 之前核销的）条目没有 kind：本地条目
                                // 只可能是 preset/skill/app，从 zip manifest 嗅探一次再定安装方式。
                                let kind = match.kind;
                                if (kind !== 'theme' && kind !== 'tool' && kind !== 'skill' && kind !== 'preset' && kind !== 'app') {
                                ...
                                entry = {
                                    ...
                                    description: { en: '', zh: '' },
                                    install: '',
                                    // preset/skill/app 走目录拷贝安装（不走 pnpm），必须原样保留——
                                    // 曾把 preset 降级成 'plugin' 导致走 pnpm add 而失败
                                    kind: kind === 'theme' || kind === 'tool' || kind === 'skill' || kind === 'preset' || kind === 'app' ? kind : 'plugin',
                        ...
                        // 下架拦截（黑名单 fail-open）：主防线是平台下载 404 与 registry
                        // 不再返回；这里给已缓存/已解锁条目的安装补一层显式拒绝。
                        if (await isBlacklisted(entry)) {
                            logEvent('warn', 'install-rejected', `${entry.name}: blacklisted (removed from marketplace)`);
                            sendJson(response, 409, {
                                error: '该插件已被平台下架，无法安装 / This plugin has been removed from the marketplace and cannot be installed.',
                            });
                            return;
                        }
                        ...
                        // manifest v2: kind=app packages copy into <dsh-home>/apps/<name>/ —
                        // standalone local processes, never pnpm, never mounted into DSH.
                        // Platform allows GitHub-sourced apps only, and they arrive
                        // AI-audited (import-github runs the audit on these exact dist
                        // bytes); the sha256 bound to the audit report is re-verified here
                        // so the buyer always runs the reviewed bytes.
                        if (entry.kind === 'app') {
                            try {
                                const unlockedMatch = readUnlockedState(activeProfileDir).bundles
                                    .flatMap((b) => b.items)
                                    .find((it) => it.zip === entry.zip);
                                const record = await installApp(entry, {
                                    expectedSha256: unlockedMatch?.sha256 ?? unlockedMatch?.audit?.sha256,
                                });
                                logEvent('info', 'install', `${entry.name}: app installed to ~/.dsh/apps/ (start: ${record.start})`);
                                sendJson(response, 200, {
                                    ok: true,
                                    hot: false,
                                    app: record,
                                ...
                                logEvent('error', 'install', `${entry.name}: app install failed: ${error instanceof Error ? error.message : String(error)}`);
                                sendJson(response, 502, { error: error instanceof Error ? error.message : String(error) });
                            }
                            return;
                        }
                        ...
                        let target;
                        try {
                            target = entryNeedsZip(entry)
                                ? await materializeTgz(entry, { expectedSha256: entry.checksum })
                    ...
                    const message = error instanceof Error ? error.message : String(error);
                    host.logger?.warn(`[dshhub-market] install failed: ${message}`);
                    logEvent('error', 'install', `route error: ${message}`);
                    sendJson(response, 500, { error: message });
                }
            },
        }),
        // ---- kind=app 可部署应用：部署 / 停止 / 状态 ----
        // 部署 = 在 127.0.0.1 随机空闲端口 spawn manifest.start（build 首次部署前
        // 先执行一次）。浏览器打开由 UI 层用返回的 url window.open（客户端内不能
        // 静默开窗，必须用户手势触发）。
        host.webServer.register({
            kind: 'exact',
            path: '/dsh-market/apps/deploy',
            handler: async (request, response) => {
                if (request.method !== 'POST') {
                    response.writeHead(405, { allow: 'POST' });
                    response.end();
                ...
                try {
                    await withMutationLock(response, 'write', async () => {
                        ...
                        if (name === '') {
                            sendJson(response, 400, { error: '缺少应用名' });
                            return;
                        }
                        try {
                            const result = await deployApp(name);
                            logEvent('info', 'app-deploy', `${name} → ${result.url}`);
                            sendJson(response, 200, { ok: true, ...result });
                        }
                        catch (error) {
                            const message = error instanceof Error ? error.message : String(error);
                            logEvent('warn', 'app-deploy', `${name} failed: ${message}`);
                            sendJson(response, 502, { error: message });
                        }
                    });
                }
        ...
        host.webServer.register({
            ...
            handler: async (request, response) => {
                ...
                sendJson(response, 200, { apps: listAppStatus() });
            },
        }),
    ];
    return () => {
