...
import { bindToAccount, startCloudBridge, stopCloudBridge } from './cloud-bridge.js';
/** 云轮询当前 profile（mountMarketRoutes 时记录，bridge-control 用） */
let bridgeProfile = '';
export function setBridgeProfile(profile) { bridgeProfile = profile; }
        ...
        host.webServer.register({
            ...
            handler: async (request, response) => {
                ...
                try {
                    ...
                    if (action === 'start') {
                        if (bridgeProfile !== '')
                            void startCloudBridge(bridgeProfile);
                    }
                    else if (action === 'stop') {
                        stopCloudBridge();
                    }
        ...
        host.webServer.register({
            ...
            handler: async (request, response) => {
                ...
                for (const [name, spec] of Object.entries(installed)) {
                    if (spec.startsWith('skill:') || spec.startsWith('preset:'))
                        continue;
