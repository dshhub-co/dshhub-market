...
import { FORK_SELF_NAME, fetchOwnVersion } from './self-update.js';
...
/**
 * Specs that point at a local path: `file:`/`link:` plus absolute paths —
 * the fork's zip installs record the cached tgz path directly. These update
 * from their local source, never from npm (see checkUpdates and the update
 * route's rejection below).
 */
export function isLocalPathSpec(spec) {
    return spec.startsWith('link:') || spec.startsWith('file:') || /^(\/|[A-Za-z]:[\\/])/.test(spec);
}
...
channelFor = new Map()) {
    ...
    await Promise.all(Object.entries(installed).map(async ([name, spec]) => {
        ...
        try {
            if (name === FORK_SELF_NAME) {
                // fork: the fork's updates come from the dshhub.co version endpoint,
                // not from npm (dshhub-market is never published there).
                const latest = await fetchOwnVersion();
                result[name] = {
                    kind: 'npm', version, current: version, latest,
                    updateAvailable: isUpgrade(version, latest),
                };
            }
            else if (spec.startsWith('github:') && gh !== null) {
