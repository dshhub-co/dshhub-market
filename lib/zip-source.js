...
export function entryNeedsZip(entry) {
...
function locateManifest(entries) {
    ...
    if (atRoot !== undefined) {
        return {
            manifest: JSON.parse(new TextDecoder().decode(atRoot)),
            rooted: entries,
        };
    }
 ...
 */
function ensurePackageJson(rooted, manifest) {
    if (rooted['package.json'] !== undefined)
        return rooted;
 ...
 */
export async function zipKind(url) {
    try {
 ...
 */
export async function materializeTgz(entry, opts) {
    const zipUrl = entry.zip;
    let zipBytes;
    try {
        const headers = {};
        if (opts?.token)
