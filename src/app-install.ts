...
function locateManifest(entries: Record<string, Uint8Array>): {
  ...
  if (atRoot !== undefined) {
    return { manifest: JSON.parse(new TextDecoder().decode(atRoot)) as AppManifestLike, rooted: entries }
  }
  ...
  if (topDirs.size === 1) {
    ...
    if (mf !== undefined) {
      ...
      return { manifest: JSON.parse(new TextDecoder().decode(mf)) as AppManifestLike, rooted }
    }
  }
...
function statePathAt(name: string): string {
  return join(appsRoot(), APP_STATE_DIR, `${name}.json`)
}

/** Installed app packages: package name → record. */
export function readInstalledApps(): Record<string, InstalledApp> {
  const dir = join(appsRoot(), APP_STATE_DIR)
  if (!existsSync(dir)) return {}
  const out: Record<string, InstalledApp> = {}
  for (const file of readdirSync(dir)) {
    if (file === 'runtime.json' || !file.endsWith('.json')) continue
    try {
      const parsed = JSON.parse(readFileSync(join(dir, file), 'utf8')) as InstalledApp
...
export function appSpecMap(): Record<string, string> {
...
export function isInstalledApp(name: string): boolean {
 ...
 */
export async function installApp(entry: RegistryPlugin, opts?: { expectedSha256?: string }): Promise<InstalledApp> {
  ...
  if (zipBytes.byteLength < 22) throw new Error('应用包无效（空文件）')
  ...
  if (start === '') throw new Error('manifest.json 缺少 start（部署启动命令）')
  if (start.length > 500) throw new Error('manifest.json 的 start 命令过长（≤500 字符）')
  const build = typeof manifest.build === 'string' && manifest.build.trim() !== '' ? manifest.build.trim() : null
  if (build !== null && build.length > 500) throw new Error('manifest.json 的 build 命令过长（≤500 字符）')
  const port =
    typeof manifest.port === 'number' && Number.isInteger(manifest.port) && manifest.port >= 1 && manifest.port <= 65535
      ? manifest.port
      : null
  ...
  if (count === 0) throw new Error('应用包为空')
 ...
 */
export function uninstallApp(name: string): boolean {
  const record = readInstalledApps()[name]
  if (record === undefined) return false
  // runtime.json: running instance pid/port (path shared with app-runtime.ts).
  try {
    const runtime = JSON.parse(readFileSync(runtimeStatePath(), 'utf8')) as Record<string, unknown>
