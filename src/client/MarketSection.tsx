...
function OwnerAvatar({ name, owner }: { name: string; owner: string }) {
  ...
  // fork: dshhub zip-only entries have no GitHub owner — skip the avatar
  // instead of 404ing a github.com URL for a synthetic owner.
  const isGithubOwner = /^[A-Za-z0-9_.-]+$/.test(owner) && owner !== '' && owner !== 'dshhub'
  if (failed || !isGithubOwner) {
    return (
...
export function MarketSection(props: MarketSectionProps) {
  ...
  const selfName = installed['dshhub-market'] !== undefined ? 'dshhub-market' : installed['dshmarket'] !== undefined ? 'dshmarket' : 'dsh-market'
    ...
    <div className={css.root}>
      <div className={css.head}>
        <div className={css.titleRow}>
          ...
          <a className={css.repoLink} href="https://www.dshhub.co" target="_blank" rel="noreferrer" title="dshhub-market · dshhub.co">DSHHub.co 市场</a>
          {version !== null && <span className={css.version} title={t('versionHint')}>v{version}</span>}
          {(() => {
            const self = installed['dshhub-market'] !== undefined ? 'dshhub-market' : installed['dshmarket'] !== undefined ? 'dshmarket' : 'dsh-market'
                        ...
                        : Object.entries(displayedInstalled)
                            ...
                            return (
                              <div key={name} className={missing ? `${css.irow} ${css.irowMissing}` : css.irow}>
                                ...
                                <span className={css.grow} />
                                {toggleable && (name === 'dsh-market' || name === 'dshmarket' || name === 'dshhub-market'
                                  ? (
                                ...
                                {!missing && name !== 'dsh-market' && name !== 'dshmarket' && name !== 'dshhub-market' && (
                                  removingName === name
