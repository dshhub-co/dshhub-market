...
export interface RegistryPlugin {
  ...
  replacement?: string
  /**
   * fork (dshhub): absolute URL of a hosted zip when the entry has no npm /
   * github source. Present only on dshhub-origin entries; installs of such
   * entries go through the zip materialization path (src/zip-source.ts).
   */
  zip?: string
  /** fork (dshhub): the dshhub catalog row id (uuid), for the local bridge. */
  dshhubId?: string
 ...
 */
const REGISTRY_URL = process.env.DSHM_REGISTRY_URL ?? 'https://www.dshhub.co/api/registry/plugins.json'
