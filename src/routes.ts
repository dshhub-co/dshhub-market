...
import { blacklistHasName, fetchBlacklist, isBlacklisted } from './blacklist.ts'
import {
...
import { appSpecMap, installApp, isInstalledApp, readInstalledApps, uninstallApp } from './app-install.ts'
import { appStatus, deployApp, listAppStatus, stopApp } from './app-runtime.ts'
        ...
        try {
          ...
          const upstreamBody = (await upstream.json().catch(() => ({}))) as {
            ...
            bundle?: {
              ...
              items?: (UnlockedBundleRecord['items'][number] & { status?: string })[]
            }
          }
    ...
    host.webServer.register({
      ...
      handler: async (request, response) => {
        ...
        for (const [name, spec] of Object.entries(installed)) {
          if (spec.startsWith('skill:') || spec.startsWith('preset:') || spec.startsWith('app:')) continue
        ...
        for (const [name, spec] of Object.entries(installed)) {
          ...
          else if (spec.startsWith('app:')) kinds[name] = 'app'
          else {
