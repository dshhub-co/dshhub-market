...
export function readInstalledSkills(profileDirectory: string): Record<string, InstalledSkill> {
  ...
  for (const file of readdirSync(dir)) {
    ...
    try {
      const parsed = JSON.parse(readFileSync(join(dir, file), 'utf8')) as InstalledSkill
...
export function skillSpecMap(profileDirectory: string): Record<string, string> {
...
function locateManifest(entries: Record<string, Uint8Array>): {
  ...
  if (atRoot !== undefined) {
    return { manifest: JSON.parse(new TextDecoder().decode(atRoot)) as SkillManifestLike, rooted: entries }
  }
  ...
  if (topDirs.size === 1) {
    ...
    if (mf !== undefined) {
      ...
      return { manifest: JSON.parse(new TextDecoder().decode(mf)) as SkillManifestLike, rooted }
    }
  }
...
export async function installSkill(profileDirectory: string, entry: RegistryPlugin): Promise<InstalledSkill> {
  ...
  if (zipBytes.byteLength < 22) throw new Error('技能包无效（空文件）')
  ...
  const skillDirs = Array.isArray(manifest.skills)
    ? manifest.skills.filter((s): s is string => typeof s === 'string' && s.trim() !== '').map(s => s.replace(/^\.?\//, '').replace(/\/+$/, ''))
    : ['skills']
  ...
  for (const dir of skillDirs) {
    ...
    if (mdText === null) throw new Error(`技能目录缺少 SKILL.md：${dir}`)
    ...
    if (count === 0) throw new Error(`技能目录为空：${dir}`)
...
export function isInstalledSkill(profileDirectory: string, name: string): boolean {
