...
function readRaw(dir: string): Record<string, unknown> {
  return JSON.parse(readFileSync(join(dir, '.dshhub-market', 'state.json'), 'utf8')) as Record<string, unknown>
}
...
describe('market state.json (#60)', () => {
  it('loads legacy disabledSkins; new writes use the unified disabled key', () => {
    const dir = stateDir()
    try {
      writeFileSync(join(dir, '.dshhub-market', 'state.json'), JSON.stringify({ disabledSkins: ['theme-a'] }))
  ...
  it('readMarketState normalizes malformed payloads to empty state', () => {
    const dir = stateDir()
    try {
      writeFileSync(join(dir, '.dshhub-market', 'state.json'), 'not json')
      expect(readMarketState(dir)).toEqual({ disabled: new Set(), groups: {}, groupOrder: [] })
      writeFileSync(join(dir, '.dshhub-market', 'state.json'), JSON.stringify({
        disabled: ['a', 'a', '', 7],
        groups: { work: ['x', 'x', 3] },
  ...
  it('records no channel at all until one is chosen', () => {
    ...
    try {
      ...
      writeFileSync(join(dir, '.dshhub-market', 'state.json'), JSON.stringify({ channel: 'nightly' }))
      expect(readMarketState(dir).channel).toBeUndefined()
    } finally {
      rmSync(dir, { recursive: true, force: true })
    }
